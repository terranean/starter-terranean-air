...
for i in `grep -rl terranean-wp-starter * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;terranean-wp-starter;${THEME_NAME};" $i $i; done
for i in `grep -rl terranean-wp-starter * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;terranean-wp-starter;${THEME_NAME};" $i $i; done
for i in `grep -rl air * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;terranean-wp-starter;${THEME_NAME};" $i $i; done
